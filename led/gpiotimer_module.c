...
#define DEVICE_NAME "gpiotimerled"
#define NODE_NAME "gpiotimerled"
...
static ssize_t gpio_write(struct file* inode, const char* buff, size_t len, loff_t* off) {
  ...
  if (msg[0] == '0') {
    del_timer_sync(&timer); // 타이머 정지
    gpio_set_value(GPIO_LED, 0);
  }
  else if (msg[0] == '1') {
    timer_setup(&timer, timer_func, 0); // 타이머 초기화, 만료되면 timer_func 실행.
    timer.expires = jiffies + (1 * HZ); // 현재 시각에서 1초 후 만료되도록 설정
    add_timer(&timer); // 커널 타이머 서브 시스템에 등록. 1초 뒤 timer_func 호출
  }
  else {
    return -EINVAL;
  }
